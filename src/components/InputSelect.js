...
export default ({items, selected, onSelect, placeholder}) => {
  ...
  const [ arrowAnimation ] = useState( openAnimation.interpolate({inputRange: [0, 1], outputRange: [1, -1]}) );
  ...
  return (<View style={styles.box}>
    ...
    <TouchableWithoutFeedback onPress={() => setListOpened( !listOpened )}><View>
      ...
      <Animated.View style={[ styles.box__open, {transform: [{scaleY: arrowAnimation}]} ]}><SelectArrow /></Animated.View>
    </View></TouchableWithoutFeedback>

    <Animated.View style={[ styles.box__list, listOpened ? styles.box__list_opened : {}, {opacity: openAnimation} ]}>
      {items.map((item, ind) => (

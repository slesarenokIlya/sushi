...
export default ({children, onChange, style}) => {
  ...
  const setSelected = ind => {
    scrollRef &&
      scrollRef.scrollTo({x: ITEM_WIDTH * ind, y: 0, animated: true});
  ...
  const onScroll = ({
    ...
    var _offset = (x - offset) / ITEM_WIDTH,
      curItem = selected;
    if (_offset > SWIPE_POINT) curItem = selected + (Math.floor(_offset) + 1);
    else if (_offset < -SWIPE_POINT)
      curItem = selected - (Math.floor(Math.abs(_offset)) + 1);
  ...
  return (
    <View style={[styles.swiper, style]}>
      <ScrollView
        contentContainerStyle={{paddingRight: SCROLL_PADDING}}
        ref={_setScrollRef}
        horizontal
        ...
        {children.map((item, ind) => {
          ...
          return (
            <Animated.View
              key={ind}
              style={[styles.swiper__item, {opacity}, {transform: [{scale}]}]}>
              {item}
            </Animated.View>
          );
        })}

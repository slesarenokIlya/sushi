...
export default ({navigation, route}) => {
  ...
  const { item: {title, list, weight, price, image} } = route.params;

  const contentHeight = Dimensions.get('window').height * 0.5;

  return (<Screen withoutPadding>
    <View style={styles.item}>
      <Button style={styles.item__close} onClick={() => navigation.goBack()} withoutBkg noText>
        <Arrow />
      </Button>
      ...
      <BottomScreen onClose={() => {}}
        maxHeight={contentHeight} minHeight={contentHeight}
        noClose noCover noOverScroll
        hitboxComponent={() => (<></>)}
        bottomContent={(
          <Button style={styles.item__content__button} noText onClick={() => {}}>

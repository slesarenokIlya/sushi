...
export default ({type}) => {
  return (<BottomScreen noClose minHeight={400} autoHeight
        ...
        () => (
          <View style={styles.hitbox}>
            ...
            <View style={styles.header}>
              <Text style={styles.header__title} type="bold">Система лояльности</Text>
            </View>
          </View>
        )
      )}>
    ...
    <View style={styles.content}>
      {levels[type].map(({name, percent, amount}) => (<>
        <Text style={styles.content__title}>{name}</Text>
        <Text style={styles.content__text}>Кэшбэк {percent}% , оборот {amount}</Text>
      </>))}
    </View>
  </BottomScreen>);
}
